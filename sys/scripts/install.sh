...
SERVICE_NAME="j4ne"
DOMAIN="${DOMAIN:-j4ne.example.com}"
...
setup_env() {
    ...
    sudo -u $DEPLOY_USER cp $DEPLOY_DIR/sys/config/production.env $DEPLOY_DIR/.env
...
setup_nginx() {
    ...
    cp $DEPLOY_DIR/sys/nginx/$DOMAIN.conf /etc/nginx/sites-available/
...
setup_sudoers() {
    ...
    # Allow j4ne user to restart the service without password
...
EOF
    ...
    if [ -f "/etc/nginx/sites-enabled/$DOMAIN.conf.bak" ]; then
        ...
        # Re-enable the j4ne config if it wasn't backed up
        ln -sf /etc/nginx/sites-available/$DOMAIN.conf /etc/nginx/sites-enabled/
    fi
...
main() {
    ...
    echo "   - SLACK_BOT_TOKEN and SLACK_APP_TOKEN (for Slack integration)"
