        ...
        class RaisesContext:
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                ...
                if self.match and self.match not in str(exc_val):
                    raise AssertionError(
                        f"Expected exception message to contain '{self.match}', got '{exc_val}'"
                    )
...
class TestBashTool:
    ...
    def test_command_with_working_directory(self):
        ...
        if platform.system() == "Windows":
            result = self.tool.execute(
                "dir", workdir=self.temp_dir, description="List directory"
            )
            assert "test.txt" in result["output"]
        else:
            result = self.tool.execute(
                "ls", workdir=self.temp_dir, description="List directory"
            )
            assert "test.txt" in result["output"]
    ...
    def test_command_timeout(self):
        ...
        if platform.system() == "Windows":
            # On Windows, use ping to create a delay (ping -n 5 waits ~4 seconds)
            result = self.tool.execute(
                "ping -n 5 127.0.0.1", timeout=1, description="Test timeout"
            )
        else:
            # Unix sleep command
    ...
    def test_command_failure(self):
        ...
        result = self.tool.execute(
            "nonexistent_command_12345", description="Test failure"
        )
    ...
    def test_output_truncation(self):
        ...
        if (
            len(lines) > BashTool.MAX_LINES
            or len(result["output"]) > BashTool.MAX_OUTPUT_LENGTH
        ):
    ...
    def test_stderr_capture(self):
        ...
        if platform.system() == "Windows":
            ...
            result = self.tool.execute(
                "echo Error message 1>&2", description="Test stderr"
            )
        else:
            # Unix command that outputs to stderr
            result = self.tool.execute(
                "echo 'Error message' >&2", description="Test stderr"
            )

        assert (
            "Error message" in result["output"] or "Error message" in result["stderr"]
        )
    ...
    def test_unicode_handling(self):
        ...
        if platform.system() == "Windows":
            ...
            result = self.tool.execute(
                "echo 'Hello 世界 🌍'", description="Test unicode"
            )
    ...
    def test_convenience_function(self):
        ...
        result = execute_bash_command(
            "echo 'Convenience test'", description="Test convenience"
        )
    ...
    def test_working_directory_persistence(self):
        ...
        assert (
            self.temp_dir in result["output"]
            or os.path.basename(self.temp_dir) in result["output"]
        )
    ...
    def test_command_chaining(self):
        ...
        if platform.system() == "Windows":
            result = self.tool.execute(
                "echo First && echo Second", description="Test chaining"
            )
        else:
            result = self.tool.execute(
                "echo 'First' && echo 'Second'", description="Test chaining"
    ...
    def test_failed_command_chaining(self):
        ...
        if platform.system() == "Windows":
            result = self.tool.execute(
                "nonexistent_cmd && echo Should not appear",
                description="Test failed chaining",
            )
        else:
            result = self.tool.execute(
                "false && echo 'Should not appear'", description="Test failed chaining"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Basic test - Success: {result['success']}, Output: {result['output'].strip()}"
    )
